...
`--accent-sec` 는 배경만 죽이고 글자색은 `--accent-pri` 와 같은 `fgAccent`(흰색)입니다.
Figma 는 여기에 팔레트 밖 `#EEE` 를 쓰지만 흰색으로 통일했습니다.
...
섹션은 `1200px` 에서 멈춥니다. 컨테이너가 `--wide` 로 1600 까지 넓어져도 카드는 따라가지
않습니다 — 넓은 폭은 KV 만의 것이고, 카드가 그만큼 늘어나면 이미지가 과하게 커집니다.
