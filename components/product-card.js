 ...
 *   1. sectionTitle 의 이동 버튼을 스크롤에 연결하고, 끝에 닿으면 비활성화
...
function updateButtons(track, prev, next) {
  ...
  if (prev) prev.disabled = track.scrollLeft <= EDGE_TOLERANCE
  if (next) next.disabled = track.scrollLeft >= max - EDGE_TOLERANCE
}
...
export function initProductCarousel(scope = document) {
  for (const section of scope.querySelectorAll('.product-card-section')) {
    ...
    const sync = () => { updateEdges(track); updateButtons(track, prev, next) }
    const step = () => {
    ...
    const move = (dir) => {
      const max = track.scrollWidth - track.clientWidth
      const to = Math.max(0, Math.min(max, track.scrollLeft + dir * step()))
