/**
 * 자리가 없는 일정을 고르면 신청이 아니라 빈자리 알림이 되도록 합니다.
 *
 *   import { initSeatAlert } from './components/seat-alert.js'
 *   initSeatAlert()
 *
 * 일정 목록의 버튼 하나가 세 가지 일을 합니다.
 *
 *   보통 일정        신청하기 — 결제 화면으로 갑니다.
 *   빈자리 알림 가능  빈자리 알림 받기 — 묻는 창을 띄웁니다. 결제하지 않습니다.
 *   알림 신청 완료    알림 취소 — 묻는 창을 띄웁니다.
 *
 * 버튼을 세 개 두고 감췄다 보였다 하지 않는 것은, 그 자리에 있는 것이 언제나 "지금
 * 고른 일정으로 할 수 있는 일" 하나여야 하기 때문입니다. 무엇을 하는 버튼인지는
 * 고른 일정이 정합니다.
 *
 * 이것은 줄을 서는 일이 아닙니다. 순번도 우선권도 없고, 자리가 열리면 신청한 사람
 * 모두에게 한꺼번에 알린 뒤 먼저 결제한 사람이 갑니다. 그래서 신청을 마쳐도 카드는
 * 선택된 채로 남고 잔여석 자리의 글자만 "알림 신청 완료"로 바뀝니다 — 자리를 잡아둔
 * 것처럼 보이는 표시를 여기에 두면 안 됩니다.
 *
 * 묻는 창에서 확인을 눌렀다고 그 일이 된 것은 아닙니다. 어긋날 수 있습니다.
 *
 *   신청 실패   네트워크·서버 오류. 아무것도 달라지지 않았으니 그대로 두고 알리기만
 *               합니다 — 다시 누르면 됩니다.
 *   접수 종료   그 사이에 그 일정의 알림 접수가 닫혔습니다. 이건 다시 눌러도 같은
 *               결과라, 카드를 마감으로 돌리고 선택을 다음 일정으로 옮깁니다.
 *   취소 실패   같은 네트워크·서버 오류. 알림은 걸린 채로 남습니다 — 취소된 것처럼
 *               보였다가 되살아나면 어느 쪽이 사실인지 알 수 없습니다.
 *
 * 취소에는 "종료" 짝이 없습니다. 접수가 닫히는 것은 새로 신청할 수 없다는 뜻이지,
...
const ERROR = {
  alert: '빈자리 알림 신청에 실패했어요. 잠시 후 다시 시도해 주세요.',
  closed: '빈자리 알림 신청이 종료되었어요. 다른 일정을 확인해 주세요.',
  cancel: '알림 취소에 실패했어요. 잠시 후 다시 시도해 주세요.',
}

/** 상태마다 잔여석 자리에 붙는 아이콘. 종 -> 체크로, 신청 전후가 그림으로도 갈립니다. */
const ICON = { alert: '#icon-notifications', alerted: '#icon-check' }

const warn = (text) => showToast(text, { icon: '#icon-error', tone: 'critical' })

/* 어긋나는 순간은 서버가 만듭니다. 이 프로토타입에는 서버가 없어 주소로 흉내 냅니다 —
   detail.html?alert=fail · =closed · =cancel-fail

   취소 실패를 따로 둔 것은, 신청이 늘 실패하면 취소할 알림 자체가 생기지 않기
   때문입니다. cancel-fail 에서는 신청은 되고 취소만 어긋납니다. */
const trouble = () => new URLSearchParams(location.search).get('alert')

export function initSeatAlert(scope = document) {
  ...
  /** 알림을 걸어둔 일정. 이 화면에서는 하나만 걸 수 있습니다. */
  let alerted = null

  const cardOf = (el) => el.closest('.option-card')
  const picked = () => options.find((el) => el.checked)
  const canAlert = (el) => cardOf(el)?.classList.contains('option-card--alert')

  /** 지금 고른 일정으로 할 수 있는 일. */
  const mode = () => {
    const el = picked()
    if (!el) return 'apply'
    if (alerted && el.value === alerted) return 'cancel'
    return canAlert(el) ? 'alert' : 'apply'
  }
  ...
  const paint = () => {
    ...
    // 알림은 결제 화면으로 가지 않습니다. 링크를 그대로 두면 새 탭으로 열어 결제
    ...
    if (now === 'apply') {
      ...
      link.setAttribute('data-confirm-open', now === 'alert' ? 'seat-alert-apply' : 'seat-alert-cancel')
    }
  }
  ...
  const mark = (el, on) => {
    ...
    card.classList.toggle('option-card--alerted', on)
    // 아이콘도 함께 갑니다. 글자만 바꾸면 종이 그대로 남아, 아직 신청하지 않은 일정과
    // 그림이 같습니다.
    seats.querySelector('use')?.setAttribute('href', on ? ICON.alerted : ICON.alert)
    if (on) {
      seats.dataset.seats = seats.dataset.seats || label.textContent
      label.textContent = '알림 신청 완료'
  ...
  /**
   * 알림 접수가 닫힌 카드. 목록에서 지우지 않고 마감으로 돌립니다 — 그 시간대가
   * 있었다는 사실은 남아야 다음 일정을 고를 때 판단이 됩니다. 다시 고를 수는 없습니다.
   */
  const closeAlert = (el) => {
    const card = cardOf(el)
    const seats = card?.querySelector('.option-card__seats')
    const label = seats && labelNode(seats)
    if (!card || !label) return
    card.classList.remove('option-card--alert')
    card.classList.add('option-card--soldout')
    // 마감 카드에 적히는 말은 하나입니다. 알림 접수가 닫혔든 자리가 다 찼든, 고를 수 없다는
  ...
  document.addEventListener('confirm:accept', (e) => {
    ...
    if (id === 'seat-alert-apply') {
      ...
      alerted = el.value
      mark(el, true)
    } else if (id === 'seat-alert-cancel') {
      if (trouble() === 'cancel-fail') {
        // 알림은 걸린 채로 남습니다. 카드도 CTA(알림 취소)도 그대로입니다.
        warn(ERROR.cancel)
        return
      }
      mark(el, false)
